...
export async function POST(req: Request) {
  try {
    ...
    if (withdrawal.status === "failed" && status === "failed") {
      return NextResponse.json({ error: "Withdrawal already failed and was already handled." }, { status: 400 });
    }
    ...
    if (status === "failed") {
      if (withdrawal.refunded_at) {
        return NextResponse.json(
          { error: "This withdrawal has already been refunded." },
          { status: 400 }
        );
      }
